...
amass enum -active -timeout 5 -brute -min-for-recursive 2 -max-dns-queries 500 -d $1 -o $13.txt -r 8.8.8.8,1.1.1.1,8.8.4.4,1.0.0.1 >/dev/null 2>&1
...
#cat $1domains.txt | httpx -silent -status-code -title -content-length -follow-redirects -t 500 -timeout 30 > $1httpsubsl1.txt
...
cat $1httpsubs2.txt | grep $1 > $1https.txt

echo "[+] Removing https or http from domains [+]"
cat $1https.txt |sed -E 's/^\s*.*:\/\///g' >> $1subs.txt

echo "[+] Creating and saving ip list [+]"
(cat "$1subs.txt" | while read domain; do dig +timeout=4 +short "$domain"; done | grep -E '^[0-9]+\.[0-9]+\.[0-9]+\.[0-9]+$' | sort -u > "$1ips.txt")
...
echo "[+] Check Manually SubTakeover with https://github.com/EdOverflow/can-i-take-over-xyz [+]"
